        ...
        with col2:
            ...
            with st.expander("🎯 快速开始", expanded=True):
                ...
                1. **输入代码**
                   ...
                   - 期货示例: `CU99` (沪铜指数), `IF99` (沪深300指数), `RB99` (螺纹钢指数)

                   ⚠️ **重要提示**: 输入代码后，请按 **回车键** 确认输入！
                ...
                - **多市场支持**: 系统支持A股、美股、港股和期货分析
                - **代码格式**: 
                  - A股使用6位数字代码 (如 `000001`)
                  - 期货使用指数合约代码 (如 `CU99`, `IF99`)
                - **实时数据**: 获取最新的市场数据和新闻
            ...
            with st.expander("❓ 常见问题"):
                ...
                **Q: 可以分析期货吗？**
                A: 可以，选择"期货"市场类型，输入指数合约代码如 `CU99`、`IF99` 等。

                **Q: 期货代码格式是什么？**
                A: 期货使用指数合约代码，如 `CU99`(沪铜)、`IF99`(沪深300)、`RB99`(螺纹钢) 等。
